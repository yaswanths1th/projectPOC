...
function ManageUsers() {
  ...
  const checkPermission = useCallback(async (codename) => {
    try {
      const res = await api.get("/api/permissions/has_permission/", {
        params: { codename },
      });
  ...
  const loadRoles = useCallback(async () => {
    try {
      ...
      if (!isAdmin && departmentId) {
        all = all.filter((r) => Number(r.department) === Number(departmentId));
      }
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      let filteredUsers = raw;

      if (!isAdmin && departmentId) {
        filteredUsers = raw.filter(
          (u) => Number(u.department) === Number(departmentId)
        );
      }
  ...
  }, [roles, canViewUser, isAdmin, departmentId]);
          ...
          {loading ? (
            ...
            current.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  <span
                    className={`role-badge ${String(u.role || "").toLowerCase().replace(/\s+/g, "-")}`}
                  >
                    {u.role}
                  </span>
                </td>
