...
// --- Auto-detect error codes without hardcoding text ---
const backendErrors = Object.values(resp).map((x) => String(x).trim());

// Check if any returned value matches ANY error_code
for (const errObj of tables.user_error) {
  const code = errObj.error_code;

  // If backend returned this code as a string (exact match)
 if (resp && typeof resp === "object") {
  for (const field of Object.keys(resp)) {
    const raw = resp[field];

    // ALWAYS convert array or string to first message
    const text = Array.isArray(raw) ? String(raw[0]) : String(raw);

    // USERNAME uniqueness → map to EP016
    if (field === "username") {
      newErrs.username = await getErrorText("EP016");
      continue;
    }

    // EMAIL uniqueness → map to ES003
    if (field === "email") {
