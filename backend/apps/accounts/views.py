...
class ProfileAPIView(APIView):
    ...
    def get(self, request):
        ...
        # enrich response EXACTLY like login response
        data.update({
            "id": user.id,
            "username": user.username,
            "email": user.email,
...
class AdminUserListCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        """
        Global admin (is_superuser/is_staff) -> all users
        Department admin (non-staff)       -> only users in same department
        """
        ...
        # Department-based admin - only same department
        if user.department_id:
            return qs.filter(department_id=user.department_id)

        # No department -> no users
...
class AdminUserStatsAPIView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        # Department Admin sees ONLY department users
        if user.department_id:
            return Response({
                "total_users": User.objects.filter(department_id=user.department_id).count(),
                "active_users": User.objects.filter(is_active=True, department_id=user.department_id).count(),
                "hold_users": User.objects.filter(is_active=False, department_id=user.department_id).count(),
            }, status=200)
...
class RoleListCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        """
        Global admin -> all roles
        Department admin -> only roles under their department
        """
        qs = Role.objects.all().order_by("role_name")
        user = self.request.user

        # For GET requests we want filtering by department
        ...
        # For POST etc, default to all (permission controls who can create)
        return qs
